...
<img src="{{ '/assets/ai-more-work.png' | relative_url }}" alt="AI creating more work" />

The first week after I drove home from our holiday road-trip, I opened my laptop and thought, let's give this vendor one more try. Let test it and try once more if I can improve my 'wip' game I was working on for a long time but couldn't get to the level I wanted.
...
A week later I started accepting the idea, that my new role is product owner, with an intern that does everything I want. In the following days/evenings, I started working on some more ideas in parallel. I whipped up several apps, added level screens, music, even made templates and ci/cd and was feeling like a true leader.
...
- Assuming agents will maintain the application, why not just wait with the release until this is possible?- Should we let the agent dumb down code, so that it is easy to read for a human? (some of the code rendings a game, and the calculations are quite impressive, but if it would contain a bug - I cannot fix it easily)
